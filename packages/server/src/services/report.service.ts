...
export const ReportService = {
  ...
  async stockReport(params: any) {
    ...
    const results = items.map(item => {
      ...
      const opening = beforeTxns.filter(t => t.itemId.toString() === id).reduce((acc, t) => {
        switch (t.type) {
      ...
      const tInDept = sum(range.filter(t => t.type === ItemTransactionType.TRANSFER_IN && t.hotelId.toString() === item.hotelId.toString()), ItemTransactionType.TRANSFER_IN);
      const tOutDept = sum(range.filter(t => t.type === ItemTransactionType.TRANSFER_OUT && t.hotelId.toString() === item.hotelId.toString()), ItemTransactionType.TRANSFER_OUT);
      const tInHotel = sum(range.filter(t => t.type === ItemTransactionType.TRANSFER_IN && t.hotelId.toString() !== item.hotelId.toString()), ItemTransactionType.TRANSFER_IN);
      const tOutHotel = sum(range.filter(t => t.type === ItemTransactionType.TRANSFER_OUT && t.hotelId.toString() !== item.hotelId.toString()), ItemTransactionType.TRANSFER_OUT);
