...
export const createItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  const created = await ItemService.createItem({ ...payload, createdBy: req.user?.username, creatorName: req.user?.name });
...
export const listItems = asyncHandler(async (req: Request, res: Response) => {
  const baseFilter = req.query;
  const scopedQuery = buildRoleBasedQuery(req.user, baseFilter);
  const items = await ItemService.list(scopedQuery);
